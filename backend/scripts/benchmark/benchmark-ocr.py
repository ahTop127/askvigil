...
def run_ocr_benchmark():
    ...
    for category, file_name in IMAGE_FILES.items():
        if not os.path.exists(file_name):
            print(f"❌ Error: File '{file_name}' not found in directory. Skipping {category}.")
            continue
        ...
        print(f"-> Executing 10x Live OCR Processing Loops...")
        adjusted_latencies = []
        ...
        for i in range(10):
            ...
            with open(file_name, "rb") as img:
                ...
                files_payload = {"file": (file_name, img, "image/jpeg" if file_name.endswith(".jpg") else "image/png")}
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        print(f"   Req {i+1}: Total={total_time:.3f}s | Isolated OCR Compute={pure_ocr_compute:.3f}s")
                    else:
                        print(f"   Req {i+1}: Failed with Status {response.status_code}")
    ...
    print("="*65)
    print("| Asset Category | File Size | Avg Compute Time | Min (Best) | Max (Worst) |")
    print("| :--- | :--- | :--- | :--- | :--- |")
    for cat, stats in report_metrics.items():
        print(f"| {cat} | {stats['size']} | {stats['avg']:.4f}s | {stats['min']:.4f}s | {stats['max']:.4f}s |")
    print("="*65)
