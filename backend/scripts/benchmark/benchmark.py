...
def run_comprehensive_benchmark():
    ...
    print("Step 2: Executing Multi-Category RAC Pipeline Scans (25 iterations per category)...")
        ...
        for i in range(25):
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(f"[{category}] Req {i+1}: Total={total_time:.3f}s | Isolated Compute={pure_server_time:.3f}s")
                else:
                    print(f"[{category}] Req {i+1}: Failed with Status {response.status_code}")
    ...
    print("| Payload Category | Avg Compute Time | Min (Best Case) | Max (Worst Case) |")
    print("| :--- | :--- | :--- | :--- |")
    for cat, stats in report_metrics.items():
        print(f"| {cat} | {stats['avg']:.4f}s | {stats['min']:.4f}s | {stats['max']:.4f}s |")
