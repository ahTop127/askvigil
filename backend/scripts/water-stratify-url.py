...
features = final_df['url'].apply(lambda x: pd.Series(
...
final_df['strat_key'] = final_df['is_malicious'].astype(str) + '_' + final_df['strat_key']
...
def balanced_stratified_sample(df, n_total):
    ...
    if len(result) < n_total:
        ...
        if not remaining.empty:
            extra = remaining.sample(n=min(n_total - len(result), len(remaining)), random_state=42)
