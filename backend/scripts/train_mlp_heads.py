...
class StratifiedDataset(Dataset):
    def __init__(self, haz_records, safe_records, attr_name, text_attr_name="clean_text"):
        """
        Args:
        ...
        def get_bin(record):
            ...
            if c_len <= 500: return "short"      # Short-form
            if c_len <= 2000: return "medium"  # Medium-form
...
async def train_and_export(mode="text", label_col="label", haz_val="spam", safe_val="ham", attr_name="text_embedding"):
    ...
    haz = await OpenDataSet.filter(**{f"{attr_name}__isnull": False, label_col: haz_val}).all()
    safe = await OpenDataSet.filter(**{f"{attr_name}__isnull": False, label_col: safe_val}).all()

    if not haz or not safe:
        print(f"CRITICAL: Insufficient data for {mode}. (Haz: {len(haz)}, Safe: {len(safe)})")
        return
    ...
    for epoch in range(15): # Increased epochs for complex URL patterns
        ...
        print(f"Epoch {epoch+1:02d} | T_Loss: {t_loss/len(train_loader):.4f} | T_Acc: {t_correct/train_size:.3f} | V_Loss: {v_loss/len(val_loader):.4f} | V_Acc: {v_correct/val_size:.3f}")
    ...
    print(classification_report(y_true, y_pred, target_names=[str(haz_val), str(safe_val)]))
