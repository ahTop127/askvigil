...
# --- SIMULATED DATASET ---
MOCK_DATA = [
    {"id": 1, "text": "Please update your Maybank account details immediately.", "cat": "Phishing"},
    {"id": 2, "text": "Tahniah! Anda menang hadiah RM10,000 dari Shopee.", "cat": "Financial Scam"},
    {"id": 3, "text": "Eh boss, your CIMB account got problem, click here update.", "cat": "Phishing (Manglish)"},
    {"id": 4, "text": "I am going to eat chicken rice with my family.", "cat": "Safe/Ham"},
    {"id": 5, "text": "Job recruitment: Earn RM500 per day working from home.", "cat": "Job Scam"},
]
...
class HybridTester:
    def __init__(self):
        self.tokenizer = AutoTokenizer.from_pretrained(str(TEXT_MODEL_PATH), local_files_only=True)
        self.session = ort.InferenceSession(str(TEXT_MODEL_PATH / "model_quantized.onnx"), providers=['CPUExecutionProvider'])
    ...
    def _get_vec(self, text):
        encoded = self.tokenizer(text, padding=True, truncation=True, max_length=512, return_tensors='np')
    ...
    async def run_hybrid_search(self, query):
        ...
        for item in MOCK_DATA:
            id_ = item["id"]
            # 1 / (k + rank)
            score = (1 / (K_CONSTANT + sem_ranks[id_])) + (1 / (K_CONSTANT + lex_ranks[id_]))
