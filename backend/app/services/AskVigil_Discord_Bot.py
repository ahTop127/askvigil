...
ASKVIGIL_API_URL = os.getenv("ASKVIGIL_API_URL")
...
class Client(discord.Client):
    ...
    async def on_guild_join(self, guild): #we will introduce our bot when it is added to a new server
        intro_message = (
            "👋 **Welcome to AskVigil!**\n\n"
    ...
    async def on_message(self, message):
        ...
        if message.content.startswith('hello'):
            await message.channel.send(f'Hi there {message.author}, welcome to **AskVigil!** Type `!help` to see how I can help you stay safe from scams.')
        ...
        if message.content.startswith("!scan"): #scanning
            ...
            result = await self.scan_with_askvigil(text=text_to_scan,attachment=image_attachment)
    ...
    def format_text_result(self, unified, text_analysis, url_analysis, has_url_analysis):
        ...
        if not dont_do_text:
            dont_do_text = "- Avoid sharing sensitive information or clicking unfamiliar links."
    ...
    def build_url_section(self, url_analysis): #url under the text section
        ...
        for index, url_item in enumerate(url_analysis[:3], start=1):
            ...
            if url_percent >= 69:
                url_emoji = "🚨"
                url_level = "High"
                url_summary = "This URL shows strong technical signs of being suspicious."
            elif url_percent >= 40:
                url_emoji = "⚠️"
                url_level = "Medium"
    ...
    def build_aftercare_view(self, result):
        ...
        #QR-only result
            ...
            highest_qr_risk = max(
                [item.get("risk_score", 0) for item in qr_url_analysis],
                default=0
            )
        ...
        #URL-only result
            ...
            highest_url_risk = max(
                [item.get("risk_score", 0) for item in url_analysis],
                default=0
            )
