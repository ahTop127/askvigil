    ...
    for text in texts:
        ...
        for i in range(len(encoded["input_ids"])):
            ...
            if "token_type_ids" in encoded:
                inputs["token_type_ids"] = encoded["token_type_ids"][i:i+1].astype(np.int64)
...
async def scan_url(raw_url: str):
    ...
    output = await asyncio.to_thread(session.run, None, {"input": vector.reshape(1, -1)})
    ...
    return {
        ...
        "decision": "flagged" if risk_score > 0.8 else "clear", # Higher threshold for URLs
        "evidence": {
...
async def scan_unified_text(raw_text: str):
    ...
    for url in urls:
        ...
        results["overall_risk_score"] = max(results["overall_risk_score"], url_res["risk_score"])
