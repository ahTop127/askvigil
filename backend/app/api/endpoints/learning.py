...
from app.schemas.quiz import QuizQuestionOut, ScamCategoryOut, QuizBatchSubmitIn, QuizBatchSummaryOut
from app.services import learning_svc

router = APIRouter()

@router.get("/categories", response_model=List[ScamCategoryOut], summary="Get all fraud categories")
async def fetch_scam_categories():
    categories = await learning_svc.get_all_scam_categories()
    return categories

@router.get(
        "/quizzes/{category_id}", response_model=List[QuizQuestionOut],
...
async def fetch_quiz_questions(
    ...
    questions = await learning_svc.get_random_quiz_question(category_id=category_id, limit=5)
    return questions
