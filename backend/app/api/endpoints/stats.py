...
@router.get(
    ...
    response_model=RiskLevelDistributionOut,
    summary="Risk level distribution for donut/stacked bar chart",
)
...
@router.get(
    "/scam-type-ranking",
    response_model=ScamTypeRankingOut,
    summary="Scam type ranking for horizontal bar chart",
)
async def get_scam_type_ranking(
    top_n: int = Query(10, ge=1, le=50, description="Top N categories"),
):
    return await stats_svc.get_scam_type_ranking(top_n=top_n)
