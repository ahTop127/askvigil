...
class ScamCase(Model):
    ...
    source = fields.CharField(max_length=512, null=True, description="Source information (such as Bernama, The Star)")
    url_link = fields.CharField(max_length=512, null=True, description="Link to the case")

    # Filtering and classification dimensions (add index=True to improve query performance)
    scam_type = fields.CharEnumField(ScamTypeEnum, index=True, description="Types of fraud")
    # field column, add index can search quickly
    platform = fields.CharEnumField(PlatformEnum, index=True, description="The platforms where the incident occurred (such as TikTok, WhatsApp")
