...
class OpenDataSet(Model):
    ...
    raw_length = fields.IntField(default=0, description="Character length of original text")
    clean_length = fields.IntField(default=0, description="Standardize the length of the text after cleaning")
...
class PhishingURL(Model):
    ...
    is_malicious = fields.BooleanField(
        default=True, description="True=Phishing website, False=Safe website"
    )

    original_url = fields.TextField(
        description="The original URL might be a bit.ly short link"
    )
    resolved_url = fields.TextField(
        null=True, description="Redirect the real long link after expansion"
    )
    ...
    domain = fields.CharField(
        ...
        index=True,
        null=True,
        description="Clean the extracted core domain names, such as scam.com",
    )
    path = fields.TextField(null=True, description="URL path, such as /login")

    preview_title = fields.CharField(
        max_length=500,
        null=True,
        description="The title of the captured web page (used for Link Preview)",
    )

    raw_length = fields.IntField(default=0, description="Character length of original url")
    clean_length = fields.IntField(default=0, description="Standardize the length of the url after cleaning")
